...
@Composable
private fun ManualCodeInputTopBar(onBackClicked: () -> Unit) {
    ProtonTopAppBar(
...
@Composable
private fun ManualCodeInputState.errorTextForCodeInputField(): String? =
    when (val state = this as? ManualCodeInputState.Error) {
        is ManualCodeInputState.Error.EmptyCode -> stringResource(R.string.presentation_field_required)
        is ManualCodeInputState.Error.InvalidCode -> state.message ?: stringResource(R.string.edm_code_not_recognized)
        null -> null
    }
