...
fun SecurityKeysList(
    ...
    viewModel: SecurityKeysViewModel? = hiltViewModelOrNull()
) {
...
@Composable
internal fun SecurityKeyRow(key: Fido2RegisteredKey) {
    ListItem(
        text = {
            Text(text = key.name)
        },
...
@Composable
internal fun SecurityKeyError(message: String? = null) {
    Text(
        modifier = Modifier
            .padding(
                horizontal = ProtonDimens.DefaultSpacing,
            ...
            .fillMaxWidth(),
        text = message ?: stringResource(id = R.string.auth_fido_security_keys_error)
    )
}
    ...
    ProtonTheme {
        SecurityKeysList(
            ...
            state = SecurityKeysState.Success(
                keys = listOf(
                    Fido2RegisteredKey("format", UByteArray(size = 10), "Test key 1"),
                    Fido2RegisteredKey("format", UByteArray(size = 10), "Test key 2"),
                    Fido2RegisteredKey("format", UByteArray(size = 10), "Test key 3")
                )
            )
        )
    }
