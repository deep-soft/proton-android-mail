...
import me.proton.android.core.auth.presentation.secondfactor.fido2.keys.SecurityKeysAction.Load
import me.proton.android.core.auth.presentation.secondfactor.fido2.keys.SecurityKeysRoutes.Arg.getUserId
import me.proton.android.core.auth.presentation.secondfactor.fido2.keys.SecurityKeysState.Error
import me.proton.android.core.auth.presentation.secondfactor.fido2.keys.SecurityKeysState.Loading
...
class SecurityKeysViewModel @Inject constructor(
    ...
    override fun onAction(action: SecurityKeysAction): Flow<SecurityKeysState> {
        return when (action) {
            is Load -> observeState(userId)
        }
    }

    private fun observeState(userId: UserId): Flow<SecurityKeysState> = flowOf()
//        observeRegisteredSecurityKeys(userId).mapLatest {
//            SecurityKeysState.Success(it)
//        }
}

// should be replaced with entity defined in Rust
