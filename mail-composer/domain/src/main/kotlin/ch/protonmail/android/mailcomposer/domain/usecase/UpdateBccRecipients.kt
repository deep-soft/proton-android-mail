...
class UpdateBccRecipients @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        recipientsToAdd.forEach { addRecipient ->
            Timber.d("draft-recipients (BCC): adding $addRecipient")
            draftRepository.saveBccRecipient(userId, messageId, addRecipient)
                .onLeft { raise(it) }
        }

        recipientsToRemove.forEach { removeRecipient ->
            Timber.d("draft-recipients (BCC): removing $removeRecipient (NOT YET IMPLEMENTED)")
        }
