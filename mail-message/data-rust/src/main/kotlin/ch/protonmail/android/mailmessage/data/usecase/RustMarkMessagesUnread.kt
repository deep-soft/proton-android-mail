...
import uniffi.proton_mail_uniffi.MarkMessagesUnreadResult
...
class RustMarkMessagesUnread @Inject constructor() {
    ...
    suspend operator fun invoke(mailbox: MailboxWrapper, messageIds: List<LocalMessageId>): Either<DataError, Unit> =
        when (val result = markMessagesUnread(mailbox.getRustMailbox(), messageIds)) {
            is MarkMessagesUnreadResult.Error -> result.v1.toDataError().left()
            MarkMessagesUnreadResult.Ok -> Unit.right()
        }
}
